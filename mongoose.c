...
//jia.he
static void MD5_Update(MD5_CTX *ctx, const unsigned char *buf, size_t len) {
  uint32_t t;
...
static void mg_destroy_conn(struct mg_connection *conn, int destroy_if) {
  ...
  //jia.he may add flags.
#if MG_ENABLE_SSL
//extern int myFlags ;
  //if((conn->flags & MG_F_DUP2ASYN)!= MG_F_DUP2ASYN){
  //if(myFlags != 1){
  //  printf("releas ssl conn \n");
...
//jia.he todo the ssl accept action?
...
static const char *mg_version_header = "http_file_server/" MG_VERSION;
...
static void mg_send_directory_listing(struct mg_connection *nc, const char *dir,
  ...
  if(opts->directory_listing_insert_js){
  	mg_printf_http_chunk(nc,
                       "</tbody><tr><td colspan=3><hr></td></tr>\n"
                       "</table>\n"
		       "<script type=\"text/javascript\" src=\"%s\" ></script>\n"
                       "</body></html>",
      			opts->directory_listing_insert_js);
  }else{
  	mg_printf_http_chunk(nc,
		 	"</tbody><tr><td colspan=3><hr></td></tr>\n"
                       "</table>\n"
